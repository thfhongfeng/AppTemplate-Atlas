...
public class ZoomImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (zoomed && event.getPointerCount() == 1) {
                    // 只有单指按在屏幕上移动时，为拖动状态
            ...
            case MotionEvent.ACTION_UP:
                ...
                lastXMove = -1;
                lastYMove = -1;
                if (event.getPointerCount() == 1 && zoomed) {
                ...
                if (isClick && event.getPointerCount() == 1 && !zoomed)
                    performClick();
                break;
            case MotionEvent.ACTION_DOWN:
                isClick = event.getPointerCount() == 1;
                break;
            default:
                break;
        }
    ...
    private boolean isDoubleClick(MotionEvent event) {
        if (previousEvent == null || (event.getEventTime() - previousEvent.getEventTime() > 200)) {
            return false;
        }
        float deltaX = event.getX() - previousEvent.getX();
        float deltaY = event.getY() - previousEvent.getY();
    ...
    private void initBitmap(Canvas canvas) {
        if (sourceBitmap != null) {
            ...
            if (bitmapWidth > width || bitmapHeight > height) {
                ...
                // 当图片的宽高小于屏幕宽高时，拉伸展示
                float xRatio = width / (bitmapWidth * 1.0f);
                float yRatio = height / (bitmapHeight * 1.0f);
                float ratio = xRatio > yRatio ? yRatio : xRatio;
