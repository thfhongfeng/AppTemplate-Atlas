...
public class AtlasRouterBusinessAManager implements IRouterManager {
    private static final String TAG = LogUtils.makeLogTag(AtlasRouterBusinessAManager.class);

    private static volatile AtlasRouterBusinessAManager mInstance;

    private AtlasRouterBusinessAManager() {
    }

    public static AtlasRouterBusinessAManager getInstance() {
        if (mInstance == null) {
            synchronized (AtlasRouterBusinessAManager.class) {
                if (mInstance == null) {
                    mInstance = new AtlasRouterBusinessAManager();
                }
            }
        }
        return mInstance;
    ...
    public void callUiCommand(Activity activity, final String commandName, final Bundle args, final IRouterCallback callback) {
        if (!RouterBundleSwitcher.isBundleOpen(RouterBundleKey.BUSINESS_A_BUNDLE_KEY)) {
            LogUtils.releaseLog(TAG, RouterBundleKey.BUSINESS_A_BUNDLE_KEY + " is not opened");
            if (callback != null) {
        ...
        RemoteFactory.requestRemote(RemoteTransactor.class, activity,
                new Intent("atlas.transaction.intent.action.business.BusinessABundleRemoteAction"),
                new RemoteFactory.OnRemoteStateListener<RemoteTransactor>() {
                    ...
                    public void onFailed(String errorInfo) {
                        LogUtils.releaseLog(TAG, "request " + RouterBundleKey.BUSINESS_A_BUNDLE_KEY + " onFailed");
                        if (callback != null) {
